#!/usr/bin/env python3
"""
Real Fashion MNIST Benchmark for Valkey+NGT Vector Search Integration
Tests reliability and performance with actual Fashion MNIST dataset.
"""
...
class RealFashionBenchmark:
    """Benchmark class for Valkey+NGT vector search with real Fashion MNIST data"""
    ...
    def load_fashion_mnist(self, max_samples: int = 1000) -> Tuple[np.ndarray, np.ndarray]:
        ...
        try:
            ...
            # Normalize and scale down to smaller dimensions for testing
            scaler = StandardScaler()
            X = scaler.fit_transform(X.astype(np.float32))
            
            # Reduce dimensions to 128 for testing (original is 784)
            if X.shape[1] > 128:
                # Simple dimension reduction by taking first 128 features
                X = X[:, :128]
            
            print(f"✓ Loaded {len(X)} Fashion MNIST samples with {X.shape[1]} dimensions")
            return X, y
            
        except ImportError:
            print("scikit-learn not available, using synthetic data...")
            return self.load_synthetic_data(max_samples)
        except Exception as e:
            print(f"Error loading Fashion MNIST: {e}, using synthetic data...")
            return self.load_synthetic_data(max_samples)
    
    def load_synthetic_data(self, max_samples: int = 1000) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def run_benchmark(self, dataset_size: int = 500, k: int = 10) -> List[BenchmarkResult]:
        """Run complete benchmark suite"""
        print(f"\n🚀 Starting Real Fashion MNIST Benchmark")
        print(f"Dataset: {dataset_size} samples, k={k}")
        print("=" * 60)
        ...
        # Create index
        index_name = f"fashion_mnist_real_{int(time.time())}"
    ...
    def save_results(self, filename: str = None):
        ...
        if filename is None:
            ...
            filename = f"real_fashion_mnist_benchmark_{timestamp}.json"
...
def main():
    parser = argparse.ArgumentParser(description='Real Fashion MNIST Benchmark for Valkey+NGT')
    ...
    benchmark = RealFashionBenchmark(host=args.host, port=args.port)
